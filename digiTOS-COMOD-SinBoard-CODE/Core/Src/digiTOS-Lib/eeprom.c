...
//#define _EEPROM_SIZE (_EEPROM_FLASH_PAGE_SIZE/4)
#define _EEPROM_SIZE 10

#ifdef  _EEPROM_F0_LOW_DESTINY
...
#ifdef  _EEPROM_F0_MEDIUM_DESTINY
#define		_EEPROM_FLASH_PAGE_SIZE								1024
/* Base address of the Flash sectors */
#define ADDR_FLASH_PAGE_0     ((uint32_t)0x08000000) /* Base @ of Page 0, 1 Kbytes */
#define _EEPROM_FLASH_PAGE_ADDRESS    (ADDR_FLASH_PAGE_0|(_EEPROM_FLASH_PAGE_SIZE*_EEPROM_USE_FLASH_PAGE))
...
uint32_t	EEPROMPageBackup[_EEPROM_SIZE];
...
{
	if((StartVirtualAddress+HowMuchToWrite) >	(_EEPROM_SIZE))
		return false;
	if( EE_Reads(0,(_EEPROM_SIZE),EEPROMPageBackup)==false)
		return false;
